...
def main() -> int:
    ...
    parser.add_argument(
        "--rps-per-thread",
        default="0,1000,2000,4000,6000,8000",
        help="Comma-separated list of per-thread issue rates (ops/s) (default: 0,1000,2000,4000,6000,8000).",
    )
    parser.add_argument(
        "--sei-variants",
    ...
    runs: List[RunResult] = []
    total = len(rps_per_thread_list) * args.repeats
    done = 0
    for rps_pt in rps_per_thread_list:
        if rps_pt < 0:
            raise ValueError("rps-per-thread values must be >= 0")
        for r in range(1, args.repeats + 1):
            tag = _sanitize_tag(f"{tag_prefix}.rpspt{rps_pt}.r{r}")
            ...
            print(
                f"[{done}/{total}] rps_per_thread={rps_pt} nclients={args.nclients} "
