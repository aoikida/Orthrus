...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run memcached sweep over rps-per-thread (fixed nclients) and generate a throughput plot."
    )
    ...
    parser.add_argument(
        "--rps-per-thread",
        default="0,2000,4000,6000,8000,10000,12000",
        help="Comma-separated list of per-thread issue rates (ops/s) (default: 0,2000,4000,6000,8000,10000,12000).",
    )
    parser.add_argument(
        "--sei-variants",
    ...
    parser.add_argument(
        "--tag-prefix",
        default=None,
        help="Prefix used for per-run tags (default: sweep_rpspt.<preset>.ncl<N>.<timestamp>).",
    )
    parser.add_argument(
        "--out-tag",
        default=None,
    ...
    rps_per_thread_list = _parse_int_list(args.rps_per_thread)
    ...
    tag_prefix = args.tag_prefix or f"sweep_rpspt.{args.preset}.ncl{args.nclients}.{timestamp}"
    ...
    runs: List[RunResult] = []
    total = len(rps_per_thread_list) * len(sei_variants) * args.repeats
    done = 0
    for rps_pt in rps_per_thread_list:
        if rps_pt < 0:
            raise ValueError("rps-per-thread values must be >= 0")
        for variant in sei_variants:
            if variant not in sei_series_name:
                raise ValueError(f"Unknown sei variant: {variant}")
            for r in range(1, args.repeats + 1):
                tag = _sanitize_tag(f"{tag_prefix}.rpspt{rps_pt}.sei{variant}.r{r}")
                ...
                print(
                    f"[{done}/{total}] rps_per_thread={rps_pt} nclients={args.nclients} sei_variant={variant} r={r}",
                    file=sys.stderr,
                )
    ...
    by_rpspt: Dict[int, List[RunResult]] = {}
    for rr in runs:
        by_rpspt.setdefault(rr.rps_per_thread, []).append(rr)
    ...
    for variant in sei_variants:
        ...
        for rps_pt in rps_per_thread_list:
            vals = [
                rr.sei for rr in by_rpspt.get(rps_pt, []) if rr.sei_variant == variant
            ]
    ...
    out_json = results_dir / f"memcached-throughput-vs-rps-per-thread.{out_tag}.json"
    out_csv = results_dir / f"memcached-throughput-vs-rps-per-thread.{out_tag}.csv"
    out_svg = results_dir / f"memcached-throughput-vs-rps-per-thread.{out_tag}.svg"
        ...
        json.dumps(
            {
                ...
                "rps_per_thread": rps_per_thread_list,
    ...
    with open(out_csv, "w", encoding="utf8", newline="") as f:
        w = csv.writer(f)
        w.writerow(["rps_per_thread", *series_names])
        for i, rps_pt in enumerate(rps_per_thread_list):
            row: List[object] = [rps_pt]
            for name in series_names:
                v = all_series.get(name, [None] * len(rps_per_thread_list))[i]
    ...
    _write_svg_line_chart(
        out_svg,
        title=f"memcached throughput vs rps/thread (preset={args.preset}, nclients={args.nclients})",
        x_values=rps_per_thread_list,
        series=plot_series,
        x_label="rps per thread (UPDATE/GET)",
    )
