...
def _write_svg_line_chart(
    ...
    *,
    ...
    # Axis labels
    lines.append(
        f'<text x="{plot_x0+plot_w/2:.1f}" y="{height-30}" text-anchor="middle" font-size="14">nclients</text>'
    )
    lines.append(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run memcached sweeps over nclients and generate a throughput plot."
    )
    ...
    parser.add_argument(
        "--tag-prefix",
        default=None,
        help="Prefix used for per-run tags (default: sweep_nclients.<preset>.<timestamp>).",
    )
    parser.add_argument(
        "--out-tag",
        default=None,
    ...
    tag_prefix = args.tag_prefix or f"sweep_nclients.{args.preset}.{timestamp}"
    ...
    by_nclients: Dict[int, List[RunResult]] = {}
    for rr in runs:
        by_nclients.setdefault(rr.nclients, []).append(rr)
    ...
    # Write aggregated JSON/CSV + SVG
    out_json = results_dir / f"memcached-throughput-vs-nclients.{out_tag}.json"
    out_csv = results_dir / f"memcached-throughput-vs-nclients.{out_tag}.csv"
    out_svg = results_dir / f"memcached-throughput-vs-nclients.{out_tag}.svg"
    ...
    with open(out_csv, "w", encoding="utf8", newline="") as f:
        ...
        for i, n in enumerate(nclients_list):
            row: List[object] = [n]
            for name in series_names:
                v = all_series.get(name, [None] * len(nclients_list))[i]
    ...
    _write_svg_line_chart(
        out_svg,
        title=f"memcached throughput vs nclients (preset={args.preset})",
        x_values=nclients_list,
        series=plot_series,
    )
